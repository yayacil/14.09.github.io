...
document.getElementById("bq").onclick = function() {
  ...
  var { value: jawaban } = await swals.fire({
    title: 'Isi Pesan Special untuk Aya &#128073;&#128072;', input: 'text', allowOutsideClick: false, showCancelButton: false,
  });
  if(jawaban && jawaban.length < 14){
    window.jawaban = jawaban;
    pesanwhatsapp = jawaban;
    await swals.fire('OK!', 'Kiyim Pesan Special!', 'success');
    window.location = "https://api.whatsapp.com/send?phone=&text= + Love Cantiknya aya";
    tombol();
  } else {
    await swals.fire('Semangat Skripsii ayangg', 'Boleh mengerjakan skripsi sambil nangis', 'supaya cevatt selesai ya sayangkuy<3');jawab();
  }
}
